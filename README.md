# Dairy Data Analysis Dashboard
## Project Overview

This project analyzes dairy industry data to uncover sales trends, product performance, regional demand patterns, profitability metrics, and operational insights using Power BI.

The dashboard enables business stakeholders to make data-driven decisions regarding inventory management, pricing strategies, and market expansion.

## Business Problem

The dairy company wants to:

Track sales performance
Monitor revenue and profitability
Identify top-performing products
Analyze regional demand
Improve inventory planning
Support strategic decision making

## Tools Used

Power BI
Power Query
DAX
Excel
Data Modeling
Key KPIs
Total Revenue
Total Profit
Sales Growth %
Top Products
Regional Performance
Customer Trends
## Project Metrics

| Metric | Value |
|----------|----------|
| Dataset Records | 10,000+ |
| Dashboard Pages | 4 |
| DAX Measures | 20+ |
| KPIs Tracked | 15+ |
| Visualizations | 25+ |

## Dashboard Pages

### Executive Overview

![Overview](Dashboard_Screenshots/Executive_overview.png)

### Sales Analysis

![Sales](Dashboard_Screenshots/Sales_and_Distribution_nalysis.png)

### Product Analysis

![Product](Dashboard_Screenshots/Storage_Shelf_Life_and_Expiry_Risk.png)

### Regional Performance

![Region](Dashboard_Screenshots/Farm_Performance_and_Operations.png)

## Key Insights

• Top 20% products contribute to majority revenue

• Certain regions generate significantly higher profits

• Seasonal demand fluctuations impact inventory planning

• Product mix optimization can improve profitability

## Files Included

File	Description
PBIX	Power BI Dashboard
Dataset	Source Data
Screenshots	Dashboard Preview
README	Project Documentation

### Author
Shitiz Sharma

Aspiring Data Analyst
